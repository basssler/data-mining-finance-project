"""Compare Layer 1 versus Layer 1 + Layer 3 on the same time split.

This script keeps the locked Layer 1 baseline intact and evaluates whether
SEC filing sentiment features improve the daily classification benchmark.
...
Outputs:
    ...
    outputs/comparison/layer3_comparison_predictions.parquet
"""
...
def get_layer1_layer3_panel_path() -> Path:
    """Return the daily panel that includes Layer 3 SEC sentiment."""
    return PROCESSED_DATA_DIR / "modeling" / "layer1_layer3_modeling_panel.parquet"


def get_metrics_output_path() -> Path:
    """Return the Layer 3 comparison metrics JSON path and create its folder."""
    output_dir = OUTPUTS_DIR / "comparison"
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir / "layer3_comparison_metrics.json"


def get_predictions_output_path() -> Path:
    """Return the Layer 3 comparison prediction parquet path and create its folder."""
    output_dir = OUTPUTS_DIR / "comparison"
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir / "layer3_comparison_predictions.parquet"
...
def print_summary(
    ...
    """Print a concise Layer 1 versus Layer 1 + Layer 3 summary."""
...
def main() -> None:
    """Compare Layer 1 only versus Layer 1 + Layer 3 on the same holdout split."""
    ...
    setup_frames = {
        ...
        "layer1_plus_layer3": (train_df_layer3, test_df_layer3),
    }
    ...
    print("\nSaved Layer 3 comparison outputs.")
